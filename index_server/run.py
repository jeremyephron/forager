...
log_fh = logging.FileHandler('embedding_server.log')
...
class LabeledIndexReducer(Reducer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger = logging.getLogger('embed_server.LabeledIndexReducer')
    ...
    def new(cls, n_vecs, *args, **kwargs) -> 'LabeledIndexReducer':
        ...
        def create_index_kwargs(tempdir, mult=1, metric='L2'):
            # TODO: (jeremy) fix auto_config and hardcoded numbers
            # allow auto_config to take in and overwrite other config variables
        ...
        self.logger.info(
            f'Created a full index of type {self.full_index.index_str} '
            + f'for an est. {n_vecs} vectors:\n'
            + json.dumps(self.full_index.cfg, indent=2)
        )
        ...
        self.logger.info(
            f'Created a spatial index of type {self.spatial_index.index_str} '
            + f'for an est. {n_vecs * config.SPATIAL_INDEX_MULTIPLE} vectors:'
            + '\n'
            + json.dumps(self.spatial_index.cfg, indent=2)
        )
        ...
        self.logger.info(
            'Created a full inner product index of type '
            + f'{self.full_dot_index.index_str} for an est. {n_vecs} vectors:'
            + '\n'
            + json.dumps(self.full_dot_index.cfg, indent=2)
        )
        ...
        self.logger.info(
            'Created a spatial inner product index of type '
            + f'{self.spatial_dot_index.index_str} for an est. '
            + f'{n_vecs * config.SPATIAL_INDEX_MULTIPLE} vectors:\n'
            + json.dumps(self.spatial_dot_index.cfg, indent=2)
        )
    ...
    def handle_result(self, input, output):
        ...
        if len(self.labels) % 1000 == 0:
            self.logger.info(
                f'Received embeddings for {len(self.labels)} total images'
            )
    ...
    def flush(self):
        ...
        while not should_finalize:
            ...
            if should_add_full:
                n_all = len(accumulated_full_copy)
                full_ids_all = accumulated_full_copy.keys()
                full_embs_all = accumulated_full_copy.values()
                total_size = n_all * config.EMBEDDING_DIM * (4 * BYTES)
                # TODO: (jeremy) compute batch size in config file
                batch_size = config.INDEX_TRAIN_MAX_RAM // (config.EMBEDDING_DIM * 4)
                n_batches = max(n_all // batch_size, 1)

                self.logger.info(f'Accumulated {n_all} full embeddings')
                self.logger.info(
                    f'Total size is {total_size}B and max RAM is '
                    f'{config.INDEX_TRAIN_MAX_RAM}. Processing in {n_batches} '
                    f'batches of {batch_size} embeddings'
                )
                ...
                for batch in range(n_batches):
                    ...
                    full_embs = np.zeros(
                        (n, config.EMBEDDING_DIM), dtype=np.float32
                    )
                ...
                self.logger.info(f'Added {n_all} full embeddings to index')

            if should_add_spatial:
                n_all = num_accumulated_spatial_copy
                # Lazily loaded iterables
                spatial_ids_embs_all = accumulated_spatial_copy.items()
                total_size = n_all * config.EMBEDDING_DIM * (4 * BYTES)
                # TODO: (jeremy) compute batch size in config file (reuse)
                batch_size = config.INDEX_TRAIN_MAX_RAM // (config.EMBEDDING_DIM * 4)

                self.logger.info(f'Accumulated {n_all} spatial embeddings')
                self.logger.info(
                    f'Total size is {total_size}B and max RAM is '
                    f'{config.INDEX_TRAIN_MAX_RAM}. Processing in batches of '
                    f'{batch_size} embeddings'
                )
                ...
                while n_processed < n_all:
                    ...
                    spatial_ids = []
                    to_concat = []
                    for i, embs in spatial_ids_embs_all:
                        spatial_ids += [i for _ in range(len(embs))]
                        to_concat.append(embs)
                        n = len(spatial_ids)
                        if n >= min(batch_size, n_all - n_processed):
                            break
                    
                    spatial_embs = np.zeros(
                        (n, config.EMBEDDING_DIM), dtype=np.float32
                    )
                    np.concatenate(to_concat, out=spatial_embs)

                    # (jeremy) I believe this should save memory
                    del to_concat
                    gc.collect()
                ...
                self.logger.info(f'Added {n_all} spatial embeddings to index')

            if (not should_finalize and not should_add_full 
                    and not should_add_spatial):
